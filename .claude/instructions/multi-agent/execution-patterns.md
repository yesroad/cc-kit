...
Task((subagent_type = 'explore'), (model = 'sonnet'), (prompt = '{도메인A} 분석 → .claude/temp/domain-a.md'));
Task((subagent_type = 'explore'), (model = 'sonnet'), (prompt = '{도메인B} 분석 → .claude/temp/domain-b.md'));
...
```typescript
// 코드 리뷰: sonnet (백그라운드)
Task((subagent_type = 'code-reviewer'), (model = 'sonnet'), (run_in_background = true), (prompt = '전체 변경사항 코드 리뷰'));
```
...
Task((subagent_type = 'code-reviewer'), (model = 'sonnet'), (prompt = '코드 리뷰 - 규칙 준수 확인'));
